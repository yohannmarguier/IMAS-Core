...
| ↳ HDF5 ignores `path` for leaf/structure delete — **any delete call removes the whole occurrence**, not just the named field | :265-272; src/hdf5/hdf5_writer.cpp deleteData (no path parameter at all) | — | ✓ | One tripwire covers both defective granularities, since there is no leaf-vs-structure branch to differ (`HDF5Writer::deleteData` never receives a path at all): `DeleteKnownDefects.DISABLED_Hdf5LeafDeleteLeavesSiblingIntact` + `DeleteKnownDefects.DISABLED_Hdf5LeafDeleteKeepsOccurrenceFileAndLink` + tripwire `DeleteKnownDefects.Hdf5LeafDeleteCurrentlyWipesWholeOccurrence`, which pins all three observable consequences (sibling gone, `<ids>.h5` deleted, master file left holding a dangling external link — issue #63); matrix cells skip via `Backends/DeleteMatrix.{Leaf,Structure}*/HDF5` | **xfail** |
...
| HDF5 leaf delete deletes `<ids>.h5` and leaves a dangling master-file link (issue #63) | `DeleteKnownDefects.DISABLED_Hdf5LeafDeleteKeepsOccurrenceFileAndLink` | `DeleteKnownDefects.Hdf5LeafDeleteCurrentlyWipesWholeOccurrence` |
